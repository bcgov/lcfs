...
@pytest.mark.anyio
async def test_get_all_facility_locations(repo, mock_db):
    mock_result = MagicMock()
    mock_result.all.return_value = [
        MagicMock(city="Vancouver", province_state="BC", country="Canada"),
        MagicMock(city="Edmonton", province_state="Alberta", country="Canada"),
    ]
    mock_db.execute.return_value = mock_result

    result = await repo.get_all_facility_locations()
    assert result == [
        {"city": "Vancouver", "province_state": "BC", "country": "Canada"},
        {"city": "Edmonton", "province_state": "Alberta", "country": "Canada"},
    ]
    mock_db.execute.assert_awaited_once()


@pytest.mark.anyio
async def test_get_facility_location_by_name_city(repo, mock_db):
    mock_result = MagicMock()
    mock_result.all.return_value = [
        MagicMock(city="Vancouver", province_state="BC", country="Canada"),
        MagicMock(city="Edmonton", province_state="Alberta", country="Canada"),
    ...
    result = await repo.get_facility_location_by_name(city="Van")
    assert result == ["Vancouver, BC, Canada"]


@pytest.mark.anyio
async def test_get_facility_location_by_name_province(repo, mock_db):
    mock_result = MagicMock()
    mock_result.all.return_value = [
        MagicMock(city="Vancouver", province_state="BC", country="Canada"),
        MagicMock(city="Victoria", province_state="BC", country="Canada"),
    ]
    mock_db.execute.return_value = mock_result

    result = await repo.get_facility_location_by_name(province="BC")
    assert result == ["BC, Canada"]


@pytest.mark.anyio
async def test_get_facility_location_by_name_country(repo, mock_db):
    mock_result = MagicMock()
    mock_result.all.return_value = [
        MagicMock(city="Vancouver", province_state="BC", country="Canada"),
        MagicMock(city="Austin", province_state="Texas", country="USA"),
    ]
    mock_db.execute.return_value = mock_result

    result = await repo.get_facility_location_by_name(country="Can")
    assert result == ["Canada"]


@pytest.mark.anyio
async def test_get_facility_location_by_name_no_filter_returns_empty(repo, mock_db):
    result = await repo.get_facility_location_by_name()
    assert result == []
...
# system; tests covering the old history-table-backed add_comment /
# list_comments methods were removed alongside the methods themselves.
# See lcfs/tests/internal_comment/ for the new comment-path tests.
