...
@pytest.mark.anyio
async def test_create_transfer_internal_comment_success(dbsession, internal_comment_repo):
    ...
    new_transfer_internal_comment = TransferInternalComment(**transfer_internal_comment_orm_fields)
    ...
    assert added_internal_comment.internal_comment_id == new_internal_comment.internal_comment_id
    assert added_internal_comment.comment == internal_comment_orm_fields['comment']

    result = await dbsession.execute(
        select(TransferInternalComment)
        .where(
            TransferInternalComment.internal_comment_id == new_internal_comment.internal_comment_id
        )
    )
...
@pytest.mark.anyio
async def test_create_initiative_internal_comment_success(dbsession, internal_comment_repo):
    # Create and add a new internal comment
    new_internal_comment = InternalComment(**internal_comment_orm_fields)
    dbsession.add(new_internal_comment)
    await dbsession.commit()
    ...
    new_initiative_agreement_internal_comment = InitiativeAgreementInternalComment(**initiative_agreement_internal_comment_orm_fields)
    ...
    result = await dbsession.execute(
        select(InitiativeAgreementInternalComment)
        .where(
            InitiativeAgreementInternalComment.internal_comment_id == new_internal_comment.internal_comment_id
        )
    )
...
async def test_get_internal_comments_transfer_success(dbsession, internal_comment_repo):
    ...
    assert retrieved_comment['internal_comment_id'] == internal_comment_orm_model.internal_comment_id
...
@pytest.mark.anyio
async def test_get_internal_comments_initiative_success(dbsession, internal_comment_repo):
    ...
    initiative_agreement_orm_model.current_status_id = initiative_agreement_status_orm_model.initiative_agreement_status_id
    ...
    comments = await internal_comment_repo.get_internal_comments(
        EntityTypeEnum.INITIATIVE_AGREEMENT, initiative_agreement_orm_model.initiative_agreement_id
    )
...
async def test_get_internal_comment_by_id_success(dbsession, internal_comment_repo):
    ...
    # Attempt to retrieve internal comments for the initiative agreement
    comment = await internal_comment_repo.get_internal_comment_by_id(internal_comment_orm_model.internal_comment_id)
...
async def test_update_internal_comment_success(dbsession, internal_comment_repo):
    ...
    added_internal_comment = await internal_comment_repo.get_internal_comment_by_id(internal_comment_orm_model.internal_comment_id)
