...
from lcfs.tests.transfer.transfer_payloads import transfer_orm_model, transfer_history_orm_model, transaction_orm_model, transfer_update_payload_3
...
@pytest.mark.anyio
async def test_update_transfer_recorded_order_failed(client: AsyncClient, fastapi_app: FastAPI, set_mock_user_roles):
    transfer_id = 1
    set_mock_user_roles(fastapi_app, ["GOVERNMENT", "DIRECTOR"])
    url = fastapi_app.url_path_for("update_transfer", transfer_id=transfer_id)
    response = await client.put(url, json={"current_status_id": 6})
...
async def test_update_transfer_refused_successful(
    ...
    await add_models([transfer_orm_model, transfer_history_orm_model, transaction_orm_model])

    transfer_id = 1
    set_mock_user_roles(fastapi_app, ["GOVERNMENT", "DIRECTOR"])
    url = fastapi_app.url_path_for("government_update_transfer", transfer_id=transfer_id)
