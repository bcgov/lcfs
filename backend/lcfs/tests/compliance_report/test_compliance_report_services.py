...
async def test_calculate_fuel_supply_compliance_units(summary_service):
    ...
    compliance_units = await summary_service.calculate_fuel_supply_compliance_units(report_id=1)
...
async def test_calculate_low_carbon_fuel_target_summary(summary_service):
    ...
    assert line_values['18'] == 32   # Calculated compliance units (20 + 60.8 - 48, rounded to 32)
