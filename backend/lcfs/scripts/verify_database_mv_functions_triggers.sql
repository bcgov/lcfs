...
/*
===============================================================================
-- Filename:       hybrid_production_release_2025_01_29.sql
-- Author:         Alex Zorkin
-- Date:           January 29th, 2025
--
-- Description:
-- This SQL script was executed as part of the Hybrid Production Release on
-- January 29th, 2025. Its primary purposes include:
--
-- 1. Verification:
--    a. **Materialized Views**: Ensures that all expected materialized views
--       are present in the database. It checks for their existence and reports
--       any missing views.
--
--    b. **Functions**: Validates the existence and correctness of required
--       functions, specifically ensuring that trigger functions return the
--       correct type and are present in the 'public' schema.
--
--    c. **Triggers**: Confirms that all expected triggers are associated with
--       their respective tables. It accounts for any name truncations and
--       verifies trigger existence across a comprehensive list of tables.
--
-- 2. Audit Functions Validation:
--    - Checks the existence of critical audit functions (`jsonb_diff` and
--      `generate_json_delta`) with the correct argument types to ensure
--      proper auditing capabilities.
--
-- 3. Audit Triggers Validation:
--    - Validates the existence of audit triggers on a wide range of tables,
--      considering potential name truncations imposed by PostgreSQL. This
--      ensures that all necessary audit mechanisms are in place for data
--      integrity and tracking.
--
-- 4. Maintenance Operations:
--    a. **Refreshing Materialized Views**: Refreshes all relevant materialized
--       views concurrently to ensure they contain up-to-date data without
--       locking the tables for extended periods.
--
--    b. **Updating Transfer Counts**: Updates the `count_transfers_in_progress`
--       field in the `organization` table based on the current status of
--       transfers, ensuring accurate real-time tracking of transfers in progress.
--
-- 5. Fixes:
--    - Addresses issues from previous migrations where trigger names were
--      automatically truncated by PostgreSQL. It systematically drops both the
--      original and truncated trigger names and recreates them with names that
--      avoid truncation, ensuring consistent trigger naming and functionality.
--
-- Notes:
-- - Ensure that you have appropriate backups before running this script.
-- - Execute within a transaction block where necessary to maintain atomicity.
-- - Review and adjust permissions as needed to allow script execution.
--
-- Revision History:
-- Version 1.0: Initial script creation and deployment on 2025-01-29.
--
===============================================================================
*/
```
