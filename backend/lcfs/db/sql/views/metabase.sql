...
SELECT 
    ...
    fc.category as "Fuel Category", 
    sou.ci_of_fuel as "Determining Carbon Intensity",
    COALESCE(concat(fcp.prefix, fcode.fuel_suffix), 'N/A') as "Fuel Code",
    sou.quantity_supplied as "Quantity Supplied",
    sou.units as "Units",
    sou.ci_of_fuel as "RCI (Renewable Carbon Intensity)",
    eut.name as "Expected Use",
    COALESCE(sou.rationale, 'N/A') as "Other",
    cp.description as "Compliance Period",
    org.name as "Organization Name", 
    crs.status::text as "Report Status",
    cr.version as "Report Version",
    pota.description as "Provision of the Act",
    sou.create_date,
    sou.update_date,
    sou.create_user,
    sou.update_user,
