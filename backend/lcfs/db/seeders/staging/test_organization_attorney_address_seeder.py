...
async def seed_test_organization_attorney_addresses(session):
    """
    Seeds the organization attorney addresses into the database based on actual test data,
    ...
    organization_attorney_addresses_to_seed = [
        {
            ...
            "name": "LCFS Org 1",
            "street_address": "456 Blanshard St",
        ...
        },
        {
            ...
            "name": "LCFS Org 2",
            "street_address": "781 112th Street",
        ...
        },
        {
            ...
            "name": "LCFS Org 4",
            "street_address": "678 Hazle Boulevard",
    ...
    for organization_attorney_address_data in organization_attorney_addresses_to_seed:
        # Check if the organization attorney address already exists
        existing_attorney_address = await session.execute(
        ...
        if existing_attorney_address.scalar():
            logger.info(
                f"Organization attorney address with ID {organization_attorney_address_data['organization_attorney_address_id']} already exists, skipping."
            )
            continue

        # Create and add the new organization attorney address
    ...
    await session.flush()
    logger.info(
        f"Seeded {len(organization_attorney_addresses_to_seed)} organization attorney addresses."
    )
