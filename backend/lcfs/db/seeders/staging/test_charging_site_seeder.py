...
async def seed_test_charging_sites(session):
    ...
    for idx, row in df.iterrows():
        ...
        site_data = {
            ...
            "allocating_organization_name": clean_str(
                row["Allocating Organization"], None
            ),
            "status_id": status_id,
            "site_code": site_code,
            ...
            "latitude": (
                float(row["Latitude"]) if pd.notna(row["Latitude"]) else 48.4284
            ),
            "longitude": (
                float(row["Longitude"]) if pd.notna(row["Longitude"]) else -123.3656
            ),
