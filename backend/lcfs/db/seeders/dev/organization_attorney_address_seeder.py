        ...
        for organization_attorney_address_data in organization_attorney_addresses_to_seed:
            ...
            exists = await session.execute(
                select(OrganizationAttorneyAddress).where(
                    OrganizationAttorneyAddress.organization_attorney_address_id == organization_attorney_address_data["organization_attorney_address_id"],
                )
            )
            if not exists.scalars().first():
                organization_attorney_address = OrganizationAttorneyAddress(**organization_attorney_address_data)
                session.add(organization_attorney_address)

        await session.commit()
    except Exception as e:
        logger.error("Error occurred while seeding organization attorney addresses: %s", e)
        raise
