...
class UserLoginHistory(BaseModel):
    ...
    user_login_history_id = Column(Integer, primary_key=True)
    ...
    external_username = Column(
        String(150), nullable=True, comment="BCeID or IDIR username"
    )
    ...
    is_login_successful = Column(
        Boolean,
        default=False,
        comment="True if this login attempt was successful, false on failure.",
    )
    login_error_message = Column(
        String(500), nullable=True, comment="Error text on unsuccessful login attempt."
    )
