...
organization_balance_view = Table('organization_balance_view', metadata,
    Column('organization_id', Integer, primary_key=True),
    Column('compliance_units', BigInteger, comment='The actual balance of validated Low Carbon Fuel credits held by a fuel supplier between the effective_date and the expiration_date. If expiration_date is NULL then we assume that it is the current balance.'),
)
