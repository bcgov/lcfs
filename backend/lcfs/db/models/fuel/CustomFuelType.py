...
class CustomFuelType(BaseModel, EffectiveDates, DisplayOrder):
    ...
    fuel_category_id = Column(Integer, ForeignKey('fuel_category.fuel_category_id'), nullable=False, comment="Foreign key to fuel_category table")
    provision_of_the_act_id = Column(Integer, ForeignKey('provision_of_the_act.provision_of_the_act_id'), nullable=False, comment="Foreign key to provision_of_the_act table")
    fuel_code_id = Column(Integer, ForeignKey('fuel_code.fuel_code_id'), nullable=False, comment="Foreign key to fuel_code table")
    unit = Column(String, nullable=False, comment="Units of fuel quantity")
    energy_density = Column(Float(10, 2), nullable=False, comment="Energy density of the fuel")
