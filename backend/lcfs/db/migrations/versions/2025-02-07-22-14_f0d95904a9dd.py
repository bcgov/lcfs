...
def upgrade() -> None:
    ...
    # --------------------------------------------------
    # 3) Remove the old comment columns from 'transfer' table.
    # --------------------------------------------------
...
def downgrade() -> None:
    ...
    op.add_column(
        "transfer", sa.Column("to_org_comment", sa.String(length=1000), nullable=True)
    )
    op.add_column(
        "transfer", sa.Column("gov_comment", sa.String(length=1500), nullable=True)
    )
