...
class NotionalTransferServices:
    ...
    async def convert_to_model(
        self, notional_transfer: NotionalTransferCreateSchema
    ) -> NotionalTransfer:
        """
    ...
    async def update_notional_transfer(
        ...
        existing_transfer = await self.get_notional_transfer(
        ...
        existing_transfer.address_for_service = (
            notional_transfer_data.address_for_service
        )
        existing_transfer.quantity = notional_transfer_data.quantity

        if (
            existing_transfer.fuel_category.category
            is not notional_transfer_data.fuel_category
        ):
            fuel_category = await self.fuel_repo.get_fuel_category_by_name(
