...
class AllocationAgreementServices:
    ...
    async def convert_to_model(self, allocation_agreement: AllocationAgreementCreateSchema) -> AllocationAgreement:
        ...
        return AllocationAgreement(
            **allocation_agreement.model_dump(exclude={"allocation_agreement_id", "allocation_transaction_type", "fuel_category", "fuel_type", "provision_of_the_act", "fuel_code", "deleted"}),
    ...
    async def get_table_options(self) -> AllocationAgreementTableOptionsSchema:
        ...
        return AllocationAgreementTableOptionsSchema(
            ...
            provisions_of_the_act=[ProvisionOfTheActSchema.model_validate(
                provision) for provision in table_options["provisions_of_the_act"]],
            fuel_codes=[FuelCodeSchema.model_validate(
                fuel_code) for fuel_code in table_options["fuel_codes"]],
    ...
    async def get_allocation_agreements(
        ...
        """
        allocation_agreements = await self.repo.get_allocation_agreements(compliance_report_id)
        return AllocationAgreementAllSchema(
            allocation_agreements=[
                AllocationAgreementSchema.model_validate(allocation_agreement) for allocation_agreement in allocation_agreements
            ]
        )
    ...
    async def get_allocation_agreements_paginated(
        ...
        allocation_agreements, total_count = await self.repo.get_allocation_agreements_paginated(pagination, compliance_report_id)
    ...
    @service_handler
    async def update_allocation_agreement(self, allocation_agreement_data: AllocationAgreementCreateSchema) -> AllocationAgreementSchema:
        """Update an existing Allocation agreement"""
        existing_allocation_agreement = await self.repo.get_allocation_agreement(allocation_agreement_data.allocation_agreement_id)
        if not existing_allocation_agreement:
            raise ValueError("Allocation agreement not found")

        if existing_allocation_agreement.allocation_transaction_type.type != allocation_agreement_data.allocation_transaction_type:
            existing_allocation_agreement.allocation_transaction_type = await self.repo.get_allocation_transaction_type_by_name(allocation_agreement_data.allocation_transaction_type)

        if existing_allocation_agreement.fuel_type.fuel_type != allocation_agreement_data.fuel_type:
            existing_allocation_agreement.fuel_type = await self.fuel_repo.get_fuel_type_by_name(allocation_agreement_data.fuel_type)

        if existing_allocation_agreement.fuel_category.category != allocation_agreement_data.fuel_category:
            existing_allocation_agreement.fuel_category = await self.fuel_repo.get_fuel_category_by_name(allocation_agreement_data.fuel_category)

        if existing_allocation_agreement.provision_of_the_act.name != allocation_agreement_data.provision_of_the_act:
            existing_allocation_agreement.provision_of_the_act = await self.fuel_repo.get_provision_of_the_act_by_name(allocation_agreement_data.provision_of_the_act)

        if existing_allocation_agreement.fuel_code.fuel_code != allocation_agreement_data.fuel_code:
            existing_allocation_agreement.fuel_code = await self.fuel_repo.get_fuel_code_by_name(allocation_agreement_data.fuel_code)

        existing_allocation_agreement.transaction_partner = allocation_agreement_data.transaction_partner
        ...
        updated_allocation_agreement = await self.repo.update_allocation_agreement(existing_allocation_agreement)
    ...
    @service_handler
    async def create_allocation_agreement(self, allocation_agreement_data: AllocationAgreementCreateSchema) -> AllocationAgreementSchema:
        """Create a new Allocation agreement"""
        allocation_agreement = await self.convert_to_model(allocation_agreement_data)
        created_allocation_agreement = await self.repo.create_allocation_agreement(allocation_agreement)
