...
class AllocationAgreementRepository:
    ...
    @repo_handler
    async def get_allocation_agreements(self, compliance_report_id: int) -> List[AllocationAgreementSchema]:
        """
    ...
    async def get_allocation_agreements_paginated(
        ...
        conditions = [AllocationAgreement.compliance_report_id ==
                      compliance_report_id]
        offset = 0 if pagination.page < 1 else (
            pagination.page - 1) * pagination.size
        limit = pagination.size

        query = select(AllocationAgreement).options(
            joinedload(AllocationAgreement.allocation_transaction_type),
            joinedload(AllocationAgreement.fuel_type),
            joinedload(AllocationAgreement.custom_fuel_type),
            joinedload(AllocationAgreement.fuel_category),
        ...
        result = await self.db.execute(
            query.offset(offset).limit(limit).order_by(
                AllocationAgreement.create_date.desc())
        )
        allocation_agreements = result.unique().scalars().all()
    ...
    @repo_handler
    async def get_allocation_agreement(self, allocation_agreement_id: int) -> AllocationAgreement:
        """
        Get a specific allocation agreement by id.
        """
        return await self.db.scalar(select(AllocationAgreement)
                                    .options(
            joinedload(AllocationAgreement.allocation_transaction_type),
            joinedload(AllocationAgreement.fuel_type),
            joinedload(AllocationAgreement.custom_fuel_type),
            ...
            .where(AllocationAgreement.allocation_agreement_id == allocation_agreement_id))

    @repo_handler
    async def update_allocation_agreement(self, allocation_agreement: AllocationAgreement) -> AllocationAgreement:
        """
        ...
        await self.db.flush()
        await self.db.refresh(allocation_agreement, ['fuel_category', 'fuel_type', 'allocation_transaction_type', 'provision_of_the_act', 'fuel_code'])
        return updated_allocation_agreement

    @repo_handler
    async def create_allocation_agreement(self, allocation_agreement: AllocationAgreement) -> AllocationAgreement:
        """
    ...
    async def delete_allocation_agreement(self, allocation_agreement_id: int):
        ...
        await self.db.execute(delete(AllocationAgreement).where(AllocationAgreement.allocation_agreement_id == allocation_agreement_id))
        await self.db.flush()
