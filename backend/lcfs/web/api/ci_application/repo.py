...
class CIApplicationRepository:
    ...
    @repo_handler
    async def refresh_pathways(self, ci_application: CIApplication) -> None:
        """
        Force a re-load of the parent's ``pathways`` collection so a stale
        cached relationship (left behind by raw ``delete()`` + ``db.add()``
        in ``replace_pathways``) doesn't leak into response serialization.
        """
        await self.db.refresh(ci_application, ["pathways"])
    ...
    # framework (see lcfs.web.api.internal_comment). The legacy
    # add_comment / list_comments methods that piggybacked on
    # ci_application_history.ci_application_snapshot were removed when CI
    # applications were migrated onto the shared system.
