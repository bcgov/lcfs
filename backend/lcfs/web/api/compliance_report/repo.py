...
class ComplianceReportRepository:
    ...
    @repo_handler
    async def get_review_fse_summary(
        self, compliance_report_id: int, period_start, period_end
    ) -> dict:
        """
        Return deterministic FSE pre-screen metrics from the FSE reporting view.
        """
        query = text(
            """
            SELECT
                ...
                COUNT(*) FILTER (
                    WHERE lower(COALESCE(charging_equipment_status, '')) = 'validated'
                ) AS validated_count,
                COALESCE(
                    jsonb_object_agg(
                        COALESCE(level_of_equipment, 'Unknown level'),
                        level_count
                    ) FILTER (WHERE level_of_equipment_label IS NOT NULL),
                    '{}'::jsonb
                ) AS level_counts,
                COUNT(*) FILTER (
                    WHERE is_active IS TRUE
                      AND supply_from_date::date <= :period_start
                      AND supply_to_date::date >= :period_end
            ...
            FROM (
                SELECT
                    *,
                    ...
                    COUNT(*) OVER (
                        PARTITION BY COALESCE(level_of_equipment, 'Unknown level')
                    ) AS level_count
                FROM v_fse_reporting_base
                WHERE compliance_report_id = :compliance_report_id
            ) fse
            """
        )
        ...
        logger.info(
            "compliance_report_review_query_timing",
            query_name="fse_review_summary",
            view_name="v_fse_reporting_base",
            compliance_report_id=compliance_report_id,
            duration_ms=round((perf_counter() - started_at) * 1000, 1),
        )
        return {
