...
vi.mock('@/components/BCButton', () => ({
  default: vi
    .fn()
    .mockImplementation(
      ({ children, onClick, disabled, startIcon, ref, ...props }) => {
...
vi.mock('@mui/material/CircularProgress', () => ({
  default: vi.fn(({ size }) => (
    <div data-test="circular-progress" data-size={size}>
      Loading...
    </div>
  ))
}))
...
describe('DownloadButton', () => {
  ...
  describe('Loading States', () => {
    ...
    it('switches between icons correctly', () => {
      const { rerender } = render(
        <DownloadButton {...defaultProps} isDownloading={false} />
      )
  ...
  describe('Network Failure Scenarios', () => {
    it('handles download initiation with network error', async () => {
      ...
      render(
        <DownloadButton {...defaultProps} onDownload={networkErrorDownload} />
      )
  ...
  describe('Performance Testing', () => {
    it('handles multiple rapid state changes', () => {
      ...
      for (let i = 0; i < 10; i++) {
        rerender(
          <DownloadButton {...defaultProps} isDownloading={i % 2 === 0} />
        )
      }
