...
vi.mock('../SummaryTable', () => ({
  default: ({ onCellEditStopped, data, useParenthesis, title, columns, width, ...props }) => {
      ...
      <div data-test={dataTest || "summary-table"} {...filteredProps}>
        ...
        {onCellEditStopped && (
          <button
            ...
            onClick={() => onCellEditStopped(data || [{ totalValue: 100 }, { totalValue: 200 }])}
          >
            Trigger Cell Edit
          </button>
        )}
...
vi.mock('@mui/material', () => ({
  Accordion: ({ children, defaultExpanded }) => (
    <div data-test="accordion" data-expanded={defaultExpanded}>{children}</div>
  ),
...
vi.mock('@mui/icons-material', () => ({
  ExpandMore: (props) => <div data-test="expand-more-icon" {...props}>ExpandMore</div>,
  CheckBox: (props) => <div data-test="checkbox-icon" {...props}>CheckBox</div>,
  CheckBoxOutlineBlank: (props) => <div data-test="checkbox-outline-blank-icon" {...props}>CheckBoxOutlineBlank</div>
}))
...
describe('ComplianceReportSummary', () => {
  ...
  it('renders complete summary content with all tables and elements', async () => {
    ...
    await waitFor(() => {
      ...
      // Headers
      expect(screen.getByText('report:summaryAndDeclaration')).toBeInTheDocument()
      ...
      expect(screen.getByTestId('toggle-label')).toHaveTextContent('Compare mode')
  ...
  it('does not render signing authority for government users', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('signing-authority-checkbox')).not.toBeInTheDocument()
    })
  })
  ...
  it('does not render penalty override checkbox for non-directors', async () => {
    ...
    render(<ComplianceReportSummary {...propsWithCorrectConditions} />, { wrapper })
    
    await waitFor(() => {
      expect(screen.queryByTestId('penalty-override-checkbox')).not.toBeInTheDocument()
    })
  })
  ...
  it('disables compare mode when enableCompareMode is false', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('toggle-disabled')).toHaveTextContent('disabled')
    })
  })
  ...
  it('renders and handles button cluster interactions', async () => {
    ...
    render(<ComplianceReportSummary {...propsWithExpandedButtons} />, { wrapper })
  ...
  it('renders penalty override checkbox when all conditions are met', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('penalty-override-checkbox')).toBeInTheDocument()
      expect(screen.getByText('Override penalty calculations')).toBeInTheDocument()
    })
  })
