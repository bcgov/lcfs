...
describe('EditViewComplianceReport', () => {
  ...
  it('calls deleteSupplementalReport on delete button click', async () => {
    const deleteSupplementalReportMock = vi.fn()

    vi.mocked(
      useComplianceReportsHook.useDeleteComplianceReport
    ).mockReturnValue({
      mutate: deleteSupplementalReportMock,
      isLoading: false
    })
    ...
    const mocks = setupMocks({
      reportData: {
        report: {
          ...setupMocks().reportData.report,
          supplementalInitiator: 'Supplier Supplemental' // ✅ Add supplemental indicator
        },
        chain: []
      }
    })
