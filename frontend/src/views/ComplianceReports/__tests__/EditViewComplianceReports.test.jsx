...
describe('EditViewComplianceReport', () => {
  const setupMocks = (overrides = {}) => {
    const defaultMocks = {
      ...
      useLocation: { state: {} },
      currentUser: {
        data: { organization: { organizationId: '123' }, isGovernmentUser: false },
        isLoading: false,
        hasRoles: mockHasRoles
      },
      complianceReport: {
        data: { data: { organizationId: '123', currentStatus: { status: COMPLIANCE_REPORT_STATUSES.DRAFT } } },
        isLoading: false,
        isError: false
      },
      organization: {
    ...
    vi.mocked(useCurrentUserHook.useCurrentUser).mockReturnValue(mocks.currentUser)
    vi.mocked(useComplianceReportsHook.useGetComplianceReport).mockReturnValue(mocks.complianceReport)
    vi.mocked(useOrganizationHook.useOrganization).mockReturnValue(mocks.organization)
    vi.mocked(useComplianceReportsHook.useUpdateComplianceReport).mockReturnValue({ mutate: vi.fn() })
  }
  ...
  it('renders DocumentUploadDialog for Draft status', async () => {
    setupMocks({ complianceReport: { data: { data: { currentStatus: { status: COMPLIANCE_REPORT_STATUSES.DRAFT } } } } })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
      expect(screen.getByText('Upload supporting documents for your report.')).toBeInTheDocument()
    })
  })

  it('displays an alert message when location state has a message', async () => {
    setupMocks({ useLocation: { state: { message: 'Test alert', severity: 'success' } } })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
  ...
  it('displays an error message when there is an error fetching the report', async () => {
    setupMocks({ complianceReport: { isError: true, error: { message: 'Error fetching report' } } })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
  ...
  it('displays the correct buttons for Submitted status with Analyst role', async () => {
    setupMocks({
      complianceReport: { data: { data: { currentStatus: { status: COMPLIANCE_REPORT_STATUSES.SUBMITTED } } } },
      currentUser: { data: { isGovernmentUser: true }, hasRoles: (role) => role === 'Analyst' }
    })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
  ...
  it('displays the correct buttons for Recommended by Analyst status with Compliance Manager role', async () => {
    setupMocks({
      complianceReport: { data: { data: { currentStatus: { status: COMPLIANCE_REPORT_STATUSES.RECOMMENDED_BY_ANALYST } } } },
      currentUser: { data: { isGovernmentUser: true }, hasRoles: (role) => role === 'Compliance Manager' }
    })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
      expect(screen.getByText('report:actionBtns.recommendReportManagerBtn')).toBeInTheDocument()
      expect(screen.getByText('report:actionBtns.returnToAnalyst')).toBeInTheDocument()
    })
  })

  it('displays the correct buttons for Recommended by Manager status with Director role', async () => {
    setupMocks({
      complianceReport: { data: { data: { currentStatus: { status: COMPLIANCE_REPORT_STATUSES.RECOMMENDED_BY_MANAGER } } } },
      currentUser: { data: { isGovernmentUser: true }, hasRoles: (role) => role === 'Director' }
    })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
      expect(screen.getByText('report:actionBtns.assessReportBtn')).toBeInTheDocument()
      expect(screen.getByText('report:actionBtns.returnToManager')).toBeInTheDocument()
    })
  })

  it('displays the correct buttons for Assessed status with Analyst role', async () => {
    setupMocks({
      complianceReport: { data: { data: { currentStatus: { status: COMPLIANCE_REPORT_STATUSES.ASSESSED } } } },
      currentUser: { data: { isGovernmentUser: true }, hasRoles: (role) => role === 'Analyst' }
    })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
  ...
  it('displays internal comments section for government users', async () => {
    setupMocks({ currentUser: { data: { isGovernmentUser: true }, hasRoles: () => true } })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
  ...
  it('does not display internal comments section for non-government users', async () => {
    setupMocks({ currentUser: { data: { isGovernmentUser: false }, hasRoles: () => false } })
    customRender(<EditViewComplianceReport />)
    await waitFor(() => {
      expect(screen.queryByText('report:internalComments')).not.toBeInTheDocument()
    })
  })
  ...
  it('displays AssessmentCard with history and correct status filtering', async () => {
    ...
    vi.mocked(useComplianceReportsHook.useGetComplianceReport).mockReturnValue({
      data: { data: { currentStatus: { status: COMPLIANCE_REPORT_STATUSES.ASSESSED }, history: historyMock } },
      isLoading: false,
      isError: false
    })
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('report:complianceReportHistory.AssessedBy')).toBeInTheDocument()
    })
  })
