...
export const DocumentsModellingStep = ({
  ...
  const downloadDoc = useDownloadDocument(PARENT_TYPE, ciApplicationId)
          ...
          documents.map((doc) => (
            ...
            >
              <Link
                component="button"
                underline="hover"
                sx={{ minWidth: 0, textAlign: 'left', cursor: 'pointer' }}
                noWrap
                onClick={() => downloadDoc(doc.documentId, doc.fileName)}
              >
                {doc.fileName}
              </Link>
