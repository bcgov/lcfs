...
import { TextField, Grid, FormControlLabel, Checkbox, InputLabel } from '@mui/material'
...
export const CreditMarketDetailsCard = () => {
  ...
  const { control, handleSubmit, reset, formState: { isDirty }, watch } = useForm({
    defaultValues: {
      contactName: organizationData?.credit_market_contact_name || `${currentUser?.firstName || ''} ${currentUser?.lastName || ''}`.trim(),
      phone: organizationData?.credit_market_contact_phone || organizationData?.phone || '',
      email: organizationData?.credit_market_contact_email || organizationData?.email || '',
  ...
  useEffect(() => {
    if (organizationData && currentUser) {
      reset({
        contactName: organizationData?.creditMarketContactName || `${currentUser?.firstName || ''} ${currentUser?.lastName || ''}`.trim(),
        phone: organizationData?.creditMarketContactPhone || organizationData?.phone || '',
        email: organizationData?.creditMarketContactEmail || organizationData?.email || '',
  ...
  const availableBalance = organizationData?.totalBalance || organizationData?.total_balance || 0
  ...
  const handleSave = (data) => {
    ...
    const updatePayload = {
      ...
      credits_to_sell: data.isSeller ? parseInt(data.creditsToSell, 10) || 0 : 0,
  ...
  return (
    <BCBox>
      ...
      >
        <BCWidgetCard
        title={t('creditMarket:marketDetails', 'Credit Trading Market Details')}
        color="nav"
        editButton={
            ...
            {isEditMode ? (
              ...
              <form onSubmit={handleSubmit(handleSave)}>
                ...
                >
                  ...
                  <BCBox display="flex" flexDirection="column" gap={2} alignItems="center">
                    <Controller
                      name="isSeller"
                      control={control}
                      render={({ field }) => (
                    ...
                    <BCBox>
                      ...
                      <Controller
                        ...
                        control={control}
                        rules={{
                          required: isSeller ? "Credits to sell is required when seller is selected" : false,
                          min: {
                            value: 0,
                ...
                <BCBox display="flex" gap={2} mt={3}>
                  ...
                  >
                    {updateCreditMarket.isPending ? t('common:saving', 'Saving...') : t('common:save', 'Save')}
                  </BCButton>
                  <BCButton
                    variant="outlined"
                    color="primary"
              ...
              <>
                ...
                >
                  ...
                  <BCBox display="flex" flexDirection="column" gap={2}>
                    <BCTypography variant="body4">
                      <strong>{t('creditMarket:contactName', 'Contact name')}:</strong>{' '}
                    ...
                    <BCTypography variant="body4">
                      <strong>{t('creditMarket:telephone', 'Telephone')}:</strong>{' '}
                      {organizationData?.creditMarketContactPhone || organizationData?.phone || t('common:notAvailable', 'Not available')}
                    </BCTypography>
                  </BCBox>
                  ...
                  <BCBox display="flex" flexDirection="column" gap={2}>
                    ...
                    <BCTypography variant="body4">
                      ...
                      {organizationData?.displayInCreditMarket || organizationData?.display_in_credit_market ? t('common:yes') : t('common:no')}
                    </BCTypography>

                    <BCTypography variant="body4">
                      <strong>{t('creditMarket:creditsToSell', 'Credits to sell')}:</strong>{' '}
      ...
      {/* Credit Trading Market Section - Full Width */}
      <BCBox mt={4}>
