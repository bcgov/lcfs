...
import { ORGANIZATION_STATUS, TRANSFER_STATUS } from '@/constants/statuses'
...
export const Transactions = () => {
  ...
  const handleRowClicked = useCallback((params) => {
    ...
    const isEditScenario = userOrgName === fromOrganization && status === TRANSFER_STATUS.DRAFT
  ...
  return (
    <>
      ...
      <Box display={'flex'} gap={2} mb={2}>
        {currentUser.organization?.orgStatus?.status === ORGANIZATION_STATUS.REGISTERED && <Role roles={[roles.transfers]}>
          <BCButton
            variant="contained"
            size="small"
            color="primary"
