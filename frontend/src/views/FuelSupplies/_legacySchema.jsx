...
export const fuelSupplyColDefs = (
  ...
  },
  {
    ...
    valueSetter: (params) => {
      if (params.newValue) {
        ...
        params.data.energyDensity = fuelType?.energyDensity
      }
      return true
    },
  ...
  },
  {
    field: 'fuelCategory',
    headerComponent: RequiredHeader,
    headerName: i18n.t('legacy:columnLabels.fuelClass'),
    cellEditor: AutocompleteCellEditor,
    cellRenderer: SelectRenderer,
    cellEditorParams: (params) => ({
    ...
    tooltipValueGetter: () => 'Select the fuel class from the list'
  },
  {
  ...
  },
  {
    ...
    cellEditorParams: (params) => {
      ...
      const uniqueFuelCodes = new Map()
      fuelType?.fuelCodes?.forEach((code) => {
        if (!uniqueFuelCodes.has(code.fuelCode)) {
          uniqueFuelCodes.set(code.fuelCode, code)
        }
      })
      return {
        options:
          Array.from(uniqueFuelCodes.values()).map((item) => item.fuelCode) ||
          [],
        multiple: false,
        disableCloseOnSelect: false,
        freeSolo: false,
    ...
    valueGetter: (params) => {
      ...
      const fuelCodes = Array.from(uniqueFuelCodes.values()).map(
        (item) => item.fuelCode
      )

      if (isFuelCodeScenario && !params.data.fuelCode) {
        if (fuelCodes.length === 1) {
          const singleFuelCode = Array.from(uniqueFuelCodes.values())[0]
  ...
  {
    field: 'targetCi',
    headerName: i18n.t('legacy:columnLabels.ciLimit'),
    editable: false,
    cellStyle: (params) =>
      StandardCellWarningAndErrors(params, errors, warnings, isSupplemental)
  },
  {
    field: 'ciOfFuel',
    headerName: i18n.t('legacy:columnLabels.fuelCi'),
    cellEditor: NumberEditor,
    cellEditorParams: {
      precision: 2,
      min: 0,
