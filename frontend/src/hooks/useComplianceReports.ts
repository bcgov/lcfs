...
export const useCompliancePeriod = (options: QueryOptions<unknown> = {}) => {
...
export const useComplianceReportWithCache = (reportId: any, options: QueryOptions<unknown> = {}) => {
  ...
  return {
    ...
    isPending: needsFetch ? queryResult.isPending : false,
...
export const useUpdateComplianceReport = (reportID: number | string | undefined | null, options: ExtMutationOptions<unknown, any> = {}) => {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables, context) => {
      ...
      if (clearCache) {
        queryClient.removeQueries({ queryKey: ['compliance-report', reportID] })
      } else {
