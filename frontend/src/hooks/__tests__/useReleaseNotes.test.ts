...
const mockRelease = {
  ...
  fullChangelogUrl:
    'https://github.com/bcgov/lcfs/compare/1.0.0-previous...1.0.0',
  ...
  test('falls back to the auto-generated content for fields the override leaves null', async ({
    ...
    mockGet.mockResolvedValue({
      data: [
        {
          version: '1.0.0',
          summary: 'Only the summary was edited.',
          sections: null
        }
      ]
    })
    ...
    expect(result.current.data?.[0].summary).toBe(
      'Only the summary was edited.'
    )
