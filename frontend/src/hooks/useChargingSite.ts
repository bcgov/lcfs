...
export const useGetAllocationOrganizations = (options: QueryOptions<unknown> = {}) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await client.get(apiRoutes.allocationOrganizationsSearch)
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await client.get(
        apiRoutes.getChargingSite.replace(':siteId', String(siteId ?? '')),
        {
    ...
    mutationFn: async (data: any) => {
      ...
      if (data.deleted) {
        ...
        return await client.delete(
          `${apiRoutes.saveChargingSite.replace(':orgID', String(organizationId ?? ''))}/${chargingSiteId}`,
          data
        )
