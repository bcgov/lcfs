Feature: Compliance Report Management
  ...
  Scenario: Supplier submits report and it is reviewed and approved through internal workflow
    # Supplier Workflow
    Given the user is on the login page
    And the supplier logs in with valid credentials
    When the supplier creates a new compliance report
    And the supplier navigates to the Fuel Supply page
    And the supplier enters a valid Fuel Supply record
    And the supplier signs and submits the report
    Then the report submission confirmation is shown
    And the supplier logs out
