...
*   **Database Optimization**:
    ...
    *   **Eager-loading conventions** (repository layer, see [#4099](https://github.com/bcgov/lcfs/issues/4099) and [#4101](https://github.com/bcgov/lcfs/issues/4101)):
        ...
        *   Watch for `lazy="selectin"` on the mapping: `FuelType.default_carbon_intensities` and `FuelCategory.category_carbon_intensities` are loaded that way, so every query that populates a fuel type or fuel category fires one extra `SELECT ... IN` per collection unless the query overrides it. The `FuelSupply` loaders (`_scalar_reference_options` in `fuel_supply/repo.py`) switch both off with `raiseload()`: nothing reads them off a fuel supply row, and `raiseload` (unlike `noload`) errors rather than returning an empty list if that ever changes. `Organization.ci_applications` has the same mapping-level `selectin` and is still loaded on every organization fetch.
